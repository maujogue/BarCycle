...
final class SettingsStore: ObservableObject {
  ...
  @Published var hudAutoCollapseDelay: AutoCollapseDelay
  @Published var manualAutoCollapseDelay: AutoCollapseDelay
  ...
  private init() {
    ...
    self.hudAutoCollapseDelay = AppSettings.shared.hudAutoCollapseDelay
    self.manualAutoCollapseDelay = AppSettings.shared.manualAutoCollapseDelay
  ...
  func setHudAutoCollapseDelay(_ d: AutoCollapseDelay) { AppSettings.shared.hudAutoCollapseDelay = d }
  func setManualAutoCollapseDelay(_ d: AutoCollapseDelay) { AppSettings.shared.manualAutoCollapseDelay = d }
}
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        Text(
          "⌘-drag your existing menu bar icons across the dividers to choose which ones always show, which appear only when you toggle, and which stay hidden."
        )
        .font(.system(size: 15))
        .foregroundStyle(.secondary)
        .fixedSize(horizontal: false, vertical: true)
        ...
        VStack(alignment: .leading, spacing: 14) {
          bullet(
            "Hold ⌘ (Command) and drag the menu bar separators to move your apps between the three zones."
          )
          bullet(
            "Click the BarCycle toggle (chevron / arrow) in the menu bar to collapse or expand the middle zone."
          )
...
struct SettingsContentView: View {
  ...
  private let manualDelayOptions: [AutoCollapseDelay] = [
    .five,
    .ten,
    .thirty,
    .oneMinute,
  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 24) {
        ...
        sectionHeader("Auto-collapse delay after HUD selection:")
        HStack(spacing: 6) {
          ForEach(AutoCollapseDelay.allCases) { delay in
            let isSelected = store.hudAutoCollapseDelay == delay
            Button {
              store.setHudAutoCollapseDelay(delay)
            } label: {
              Text(delay.label)
        ...
        sectionHeader("Auto-collapse delay after manual expansion:")
        HStack(spacing: 6) {
          ForEach(manualDelayOptions) { delay in
            let isSelected = store.manualAutoCollapseDelay == delay
            Button {
              store.setManualAutoCollapseDelay(delay)
            } label: {
              Text(delay.label)
                .font(.system(size: 12, weight: isSelected ? .medium : .regular))
                .frame(minWidth: 42, minHeight: 24)
          ...
          if accessibilityTrusted {
            ...
            HStack(alignment: .top, spacing: 8) {
              Image(systemName: "xmark.circle.fill")
                ...
                .font(.system(size: 16, weight: .semibold))
              Text(
                "BarCycle needs Accessibility access to drive your menu bar. If icons stop responding, re-grant the permission in System Settings."
              )
              .foregroundStyle(.secondary)
              .fixedSize(horizontal: false, vertical: true)
            }
